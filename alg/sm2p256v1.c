...
int sm2Sign(const char *priv_key_hex, uint8_t *content, uint32_t len, char *sig_hex) {
    ...
    char x_hex[64], y_hex[64];
    char *x_hex_t = mpz_get_str(NULL, 16, x), *y_hex_t = mpz_get_str(NULL, 16, y);
    unsigned int x_len = strlen(x_hex_t), y_len = strlen(y_hex_t);
    memset(x_hex, '0', 64);
    memset(y_hex, '0', 64);
    memcpy(&x_hex[64 - x_len], x_hex_t, x_len);
    memcpy(&y_hex[64 - y_len], y_hex_t, y_len);

    char hash_hex[65] = {'\0'};
    getUserIdBase(base_bytes);
    calHexToBytes(x_hex, 64, x_bytes);
    calHexToBytes(y_hex, 64, y_bytes);
